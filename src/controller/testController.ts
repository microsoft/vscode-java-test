...
import { isTestJavaFile, progressProvider } from '../extension';
...
import { IRunOption, ITestController, runnableTag } from './types';
import { findDirectTestChildrenForClass, findTestPackagesAndTypes, findTestTypesAndMethods, loadJavaProjects, resolvePath, synchronizeItemsRecursively, updateItemForDocument, updateItemForDocumentWithDebounce } from './utils';

@injectable()
export class JavaTestController implements ITestController {
    ...
    constructor() {
        this.watchers = [];
        this.testController = tests.createTestController('javaTestController', 'Java Test');
        this.testController.resolveHandler = this.loadChildren.bind(this);

        this.testController.createRunProfile('Run Tests', TestRunProfileKind.Run, this.runHandler.bind(this), true, runnableTag);
        this.testController.createRunProfile('Debug Tests', TestRunProfileKind.Debug, this.runHandler.bind(this), true, runnableTag);
    }

    public getControllerImpl(): TestController {
        return this.testController;
    }

    public async refresh(): Promise<void> {
        this.testController.items.forEach((root: TestItem) => {
            this.testController.items.delete(root.id);
        });

        await loadJavaProjects(this.testController);
        await this.showTestItemsInCurrentFile();
        this.startWatchingWorkspace();
    }

    public loadChildren: (item: TestItem, token?: CancellationToken) => any = instrumentOperation('java.test.explorer.loadChildren', async (_operationId: string, item: TestItem, token?: CancellationToken) => {
        if (!item) {
            await loadJavaProjects(this.testController);
            return;
        }
        ...
        if (data.testLevel === TestLevel.Project) {
            ...
            synchronizeItemsRecursively(this.testController, item, packageAndTypes);
            ...
            synchronizeItemsRecursively(this.testController, item, testMethods);
        }
    });

    public runTests: (request: TestRunRequest, option: IRunOption) => any = instrumentOperation('java.test.runTests', async (operationId: string, request: TestRunRequest, option: IRunOption) => {
        ...
        const testItems: TestItem[] = await new Promise<TestItem[]>(async (resolve: (result: TestItem[]) => void): Promise<void> => {
            ...
            const result: TestItem[] = await this.getIncludedItems(request, progressToken);
            await this.expandTests(result, TestLevel.Method, progressToken);
        ...
        const run: TestRun = this.testController.createTestRun(request);
        try {
            await new Promise<void>(async (resolve: () => void): Promise<void> => {
                ...
                this.enqueueTestMethods(testItems, run);
                const queue: TestItem[][] = this.mergeTestMethods(testItems);
                for (const testsInQueue of queue) {
                    if (testsInQueue.length === 0) {
                        continue;
                    }
                    const testProjectMapping: Map<string, TestItem[]> = this.mapTestItemsByProject(testsInQueue);
                    for (const [projectName, itemsPerProject] of testProjectMapping.entries()) {
                        const testKindMapping: Map<TestKind, TestItem[]> = this.mapTestItemsByKind(itemsPerProject);
                        for (const [kind, items] of testKindMapping.entries()) {
                            ...
                            const runner: BaseRunner | undefined = this.getRunnerByContext(testContext);
                            if (!runner) {
    ...
    private async startWatchingWorkspace(): Promise<void> {
            ...
            for (const pattern of patterns) {
                ...
                this.watchers.push(
                    ...
                    watcher.onDidCreate(async (uri: Uri) => {
                        ...
                        await updateItemForDocumentWithDebounce(this.testController, uri, testTypes);
                    }),
                    watcher.onDidChange(async (uri: Uri) => {
                        await updateItemForDocumentWithDebounce(this.testController, uri);
                    }),
                    watcher.onDidDelete(async (uri: Uri) => {
                        ...
                        const belongingProject: TestItem | undefined = this.testController.items.get(projectData.id);
                        if (!belongingProject) {
                            return;
                        }
    ...
    private async runHandler(request: TestRunRequest, token: CancellationToken): Promise<void> {
        await this.runTests(request, { token, isDebug: !!request.profile?.label.includes('Debug') });
    }

    private async showTestItemsInCurrentFile(): Promise<void> {
        if (await isTestJavaFile(window.activeTextEditor?.document)) {
            ...
            await updateItemForDocument(this.testController, window.activeTextEditor!.document.uri);
        }
    }
     ...
     */
    private enqueueTestMethods(testItems: TestItem[], run: TestRun): void {
     ...
     * @returns
     */
    private async getIncludedItems(request: TestRunRequest, token?: CancellationToken): Promise<TestItem[]> {
        let testItems: TestItem[] = [];
        if (request.include) {
            testItems.push(...request.include);
        } else {
            this.testController.items.forEach((item: TestItem) => {
                testItems.push(item);
            });
        }
        if (testItems.length === 0) {
        ...
        this.removeTestInvocations(testItems);
        testItems = await this.expandTests(testItems, TestLevel.Class, token);
        const excludingItems: TestItem[] = await this.expandTests(request.exclude || [], TestLevel.Class, token);
     ...
     */
    private async expandTests(testItems: TestItem[], targetLevel: TestLevel, token?: CancellationToken): Promise<TestItem[]> {
        ...
        while (queue.length) {
            ...
            if (testLevel >= targetLevel) {
                ...
                await this.loadChildren(item, token);
     ...
     */
    private removeTestInvocations(testItems: TestItem[]): void {
        const queue: TestItem[] = [...testItems];
        while (queue.length) {
            const item: TestItem = queue.shift()!;
     ...
     */
    private mergeTestMethods(testItems: TestItem[]): TestItem[][] {
    ...
    private mapTestItemsByProject(items: TestItem[]): Map<string, TestItem[]> {
    ...
    private mapTestItemsByKind(items: TestItem[]): Map<TestKind, TestItem[]> {
    ...
    private getRunnerByContext(testContext: IRunTestContext): BaseRunner | undefined {
